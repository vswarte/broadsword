...
pub struct ClassHierarchyDescriptor {
    ...
    pub base_class_array: Offset,
}

impl ClassHierarchyDescriptor {
    pub fn from_slice(input: &[u8]) -> Self {
        let base_class_array = Offset::from(u32::from_le_bytes(input[12..16].try_into().unwrap()));
...
mod tests {
    ...
    fn from_works() {
        ...
        let chd = ClassHierarchyDescriptor::from_slice(bytes.as_slice());

        assert_eq!(chd.signature, 0x0);
        assert_eq!(chd.attributes, 0x0);
        assert_eq!(chd.base_class_count, 0x4);
        assert_eq!(chd.base_class_array.as_usize(), 0x32e7198);
    }
}
