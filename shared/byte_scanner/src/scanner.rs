...
#[derive(Default)]
struct StandardScanner;

impl Scanner for StandardScanner {
    fn scan(&self, scannable: &[u8], pattern: &MemoryPattern) -> Option<usize> {
...
mod tests {
    ...
    fn standard_scanner_finds_the_pattern_1() {
        ...
        let result = StandardScanner::default().scan(randomness, &pattern).unwrap();
    ...
    fn standard_scanner_doesnt_find_the_pattern() {
        ...
        let randomness = include_bytes!("../../../tests/random.bin");
        let result = StandardScanner::default().scan(randomness, &pattern);
